...
def build_dataloaders(mode: str, base_dir=BASE_DIR, batch_size=BATCH_SIZE,
                      seed=SEED, val_fraction=0.20, use_mask=True):
    """
    mode         : 'ptau217' or 'atrophy'
    val_fraction : fraction of train+val subjects to hold out as validation
                   (carved from the 80% train+val split; test split is unchanged)
    ...
    # 80% train+val, 20% test — then split train+val into train / val
    trainval_end = int(0.80 * len(pet_paths))
