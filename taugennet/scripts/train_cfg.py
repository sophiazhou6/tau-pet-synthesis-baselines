#!/usr/bin/env python3
"""
train_cfg.py – TauGenNet diffusion training with Classifier-Free Guidance (CFG).

During training, conditioning is randomly zeroed with probability --cfg-prob
(default 0.15), teaching the model unconditional denoising alongside conditional.

At inference, two U-Net passes are combined:
    eps_guided = eps_uncond + guidance_scale * (eps_cond - eps_uncond)

Usage
-----
# Diffusion training with CFG (AE loaded from checkpoint)
python train_cfg.py --mode atrophy --skip-ae --diff-epochs 2000 --cfg-prob 0.15

# Evaluate after training
python train_cfg.py --mode atrophy --eval-only
"""
...
def train_diffusion(ae, unet, conditioner, schedule, latent_std,
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for z0, zm, cond_data in latent_train_loader:
            ...
            # CFG dropout: zero conditioning for a random subset of the batch
            if cfg_prob > 0:
                mask = (torch.rand(B, device=device) < cfg_prob)
                ctx  = ctx * (~mask).float().view(B, 1, 1)
...
def parse_args():
    ...
    p.add_argument("--ae-checkpoint",  type=str,
                   default="results/checkpoints/ae_exp_masked/baseline/ae_checkpoint_best.pt")
    ...
    p.add_argument("--unet-channels",  type=str,  default="128,256,512",
                   help="UNet channel widths (default '128,256,512' = small arch)")
    p.add_argument("--n-transformer",  type=int,  default=1,
                   help="Transformer blocks per UNet level (default 1 = small arch)")
    ...
    p.add_argument("--guidance-scale", type=float, default=3.0,
                   help="CFG guidance scale at inference (default 3.0; 1.0 = no guidance)")
    return p.parse_args()
...
def main():
    ...
    if args.mode in ("combined", "combined_mlp"):
        ...
        # ptau217_mlp uses the same data as ptau217 (1-dim scalar conditioning)
    ...
    if args.skip_ae or args.eval_only:
        ...
        ae_losses = train_ae(ae, train_loader, args.ae_epochs, args.ae_checkpoint, device,
                             ...
                             val_every=5, figures_dir=args.figures_dir,
