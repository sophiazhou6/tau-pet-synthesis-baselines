...
def _load_test_data(mode, arch):
    ...
    for i in tqdm(range(n), desc=f"Loading {mode}"):
        ...
        # Mask the model output to the real's brain support. real is gray-matter
        # masked (zeros outside the brain); the diffusion output leaks small nonzero
        # values everywhere, which (a) renders as a black box behind the generated
        # glass brain — the low end of the "hot" colormap — and (b) inflates MAE/MSE
        # and depresses SSIM/Pearson. Masking here fixes both the figure and the metrics.
        gen_np = gen_np * (real_np > 0)
...
def _render(nii_img, cmap, vmin, vmax, title, threshold=0.0,
            symmetric_cbar=False, black_bg=False):
    """Render a glass brain to a numpy image array via a temp file.

    threshold      : voxels with |value| < threshold are hidden (declutters the MIP).
    symmetric_cbar : True for the signed error panel, False for one-sided hot panels.
    plot_abs=False : respect sign so the diverging RdBu_r colormap works on the error.
    """
    tmp = None
...
def figure_a(real_vols, gen_vols, real_norm, gen_norm, affine, mode, output_dir):
    ...
    err_thr  = float(np.percentile(np.abs(mean_err), 75))   # show only top-25% error

    # Glass brains visualise POOLED volumes (voxelwise mean across all test subjects).
    # specs: (array, cmap, vmin, vmax, threshold, symmetric_cbar, title)
    specs = [
        (mean_real, "hot",    0,         hot_vmax, 1e-6,    False,
         "Mean Real PET (SUVR)\n[pooled mean volume]"),
        (mean_gen,  "hot",    0,         hot_vmax, 1e-6,    False,
         "Mean Generated PET (SUVR)\n[pooled mean volume]"),
        (mean_err,  "RdBu_r", -err_vmax, err_vmax, err_thr, True,
         "Mean Error: Generated − Real\n[pooled mean volume]"),
    ]

    panels = [_render(_to_nifti(arr, affine), cmap, vmin, vmax, "",
                      threshold=thr_, symmetric_cbar=sym)
              for arr, cmap, vmin, vmax, thr_, sym, title in specs]
    ...
    out = os.path.join(output_dir, f"{mode}_figure_A_2.png")
...
def figure_b(real_vols, gen_vols, real_norm, gen_norm, affine, mode, output_dir):
    ...
    for row, (label, idx) in enumerate(subjects.items()):
        ...
        # specs: (array, cmap, vmin, vmax, threshold, symmetric_cbar)
        specs = [
            (r_suvr, "hot",    0,         hot_vmax, 1e-6,    False),
            (g_suvr, "hot",    0,         hot_vmax, 1e-6,    False),
            (err,    "RdBu_r", -err_vmax, err_vmax, err_thr, True),
        ]

        for col, (arr, cmap, vmin, vmax, thr_, sym) in enumerate(specs):
    ...
    out = os.path.join(output_dir, f"{mode}_figure_B_2.png")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {out}")
...
def main():
    ...
    parser.add_argument("--output-dir", default=None,
                        help="Default: results/figures/<arch>/glass_brain "
                             "(e.g. results/figures/silu/glass_brain)")
