...
CACHEDIR = os.path.expanduser("~/fig_cache"); os.makedirs(CACHEDIR, exist_ok=True)
VOLNPZ   = os.path.join(CACHEDIR, "volumes_9case.npz")
THEME    = os.environ.get("FIG_THEME", "white")
GLASSDIR = os.path.join(CACHEDIR, "glass", THEME); os.makedirs(GLASSDIR, exist_ok=True)

def cached_glass(key, vol, affine, cmap, vmin, vmax, thr=1e-3):
    p = os.path.join(GLASSDIR, f"{key}.png")
    if os.path.exists(p):
        return plt.imread(p)
    img = render_glass_white(vol, affine, cmap, vmin, vmax, thr)
    plt.imsave(p, img)
    return img

RIDS = [6516, 6449, 6374, 6911, 6880, 6702, 6545, 6840, 6976]
OUT  = os.path.expanduser(f"~/fig_glass_9case_{THEME}bg"); os.makedirs(OUT, exist_ok=True)
V = [
...
real_img = {rid: cached_glass(f"Real_{rid}", reals[rid], affine, TAU_CMAP, 0.0, VMAX) for rid in RIDS}
...
for name, gdir, rdir in V:
    ...
    for rid in RIDS:
        ...
        imgs[rid] = cached_glass(f"{name}_{rid}", g, affine, TAU_CMAP, 0.0, VMAX)
