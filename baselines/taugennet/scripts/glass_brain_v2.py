...
def _compute_affine(test_ds):
    """Scale the first subject's NIfTI affine to match the resampled VOL_SHAPE."""
...
def figure_a_glass(real_vols, gen_vols, real_norm_list, gen_norm_list,
    ...
    out = os.path.join(output_dir, "glass_A_v2.png")
...
def figure_a_slice(real_vols, gen_vols, mode, output_dir,
    ...
    out = os.path.join(output_dir, "slice_A_v2.png")
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor=BG)
    plt.close(fig)
    print(f"  Saved: {out}")
...
def figure_a_scatter(real_vols, gen_vols, pet_vmax, mode, output_dir):
    ...
    out = os.path.join(output_dir, "scatter_A_v2.png")
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor=BG)
    plt.close(fig)
    print(f"  Saved: {out}")
...
def figure_b_glass(real_vols, gen_vols, real_norm_list, gen_norm_list,
    ...
    fig.suptitle(f"{mode.upper()}  —  Subject Glass Brain  (ranked by SSIM)",
                 fontsize=13, color=FG, y=1.01)
    out = os.path.join(output_dir, "glass_B_v2.png")
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor=BG)
    plt.close(fig)
    print(f"  Saved: {out}")
...
def figure_b_slice(real_vols, gen_vols, mode, output_dir,
                   pet_vmin, pet_vmax, err_vmax, all_metrics, ranked):
    ...
    fig.suptitle(f"{mode.upper()}  —  Subject Slices  (ranked by SSIM)",
                 fontsize=12, color=FG, fontweight="bold")

    out = os.path.join(output_dir, "slice_B_v2.png")
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor=BG)
    plt.close(fig)
    print(f"  Saved: {out}")
...
def figure_b_scatter(real_vols, gen_vols, pet_vmax, mode, output_dir,
    ...
    fig.suptitle(f"{mode.upper()}  —  Subject Voxel Scatter  (ranked by SSIM)",
                 fontsize=12, color=FG)
    plt.tight_layout()

    out = os.path.join(output_dir, "scatter_B_v2.png")
    fig.savefig(out, dpi=DPI, bbox_inches="tight", facecolor=BG)
    plt.close(fig)
    print(f"  Saved: {out}")
...
def run_mode(mode, arch, output_dir, use_dk_mask=True):
    ...
    affine = _compute_affine(test_ds)
    ...
    ranked      = _rank_by_ssim(ssims)

    # ── Print ranking for cross-check with evaluate_final.py ──────────────────
    print(f"\n  SSIM ranking (compare with evaluate_final.py to verify subject alignment):")
